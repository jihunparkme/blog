...
이전 단계까지 모든 과정을 거쳐 문맥을 완벽히 파악한 벡터가 준비되었습니다. 이제 LLM은 이 벡터를 가지고 **"그래서, 다음에 올 단어는 무엇일까?"** 라는 질문에 답해야 합니다.

이 단계는 크게 두 부분으로 나뉩니다.

#### 1\. 확률 계산: 모든 단어에 점수 매기기

LLM은 자신이 알고 있는 수만 개의 모든 단어(토큰)들을 후보로 올려놓고, 다음에 등장할 확률을 각각 계산합니다.

* **Logits 생성**: 문맥을 파악한 최종 벡터를 이용해 어휘 사전의 모든 단어에 대해 '등장 가능성 점수(Logit)'를 매깁니다.
* **Softmax 변환**: 이 점수들을 보기 쉽게 \*\*확률(%)\*\*로 변환합니다. 모든 단어의 확률을 합하면 1(100%)이 됩니다.

이제 모델은 "다음에 '미래'가 올 확률은 15%, '혁신'이 올 확률은 12%..." 와 같은 확률 분포표를 갖게 됩니다.

#### 2\. 샘플링: 확률에 따라 단어 선택하기

가장 확률 높은 단어만 계속 선택하면 매우 예측 가능하고 지루한 문장이 나옵니다. 그래서 LLM은 더 창의적인 문장을 만들기 위해 다양한 **샘플링(Sampling) 전략**을 사용합니다. 마치 확률에 따라 단어를 뽑는 '단어 룰렛'을 돌리는 것과 같습니다.

* **Greedy Search**: 무조건 확률 1등 단어만 뽑습니다. (가장 안전하지만 지루함)
* **Top-K Sampling**: 확률 순위가 높은 K개(예: 50개)의 단어만 룰렛 판에 올려놓고, 그 안에서 확률에 따라 뽑습니다.
* **Top-P (Nucleus) Sampling**: 확률의 합이 P(예: 95%)가 될 때까지 상위권 단어들을 룰렛 판에 올립니다. 모델이 다음에 올 단어를 확신할수록 후보가 적어지고, 불확실할수록 후보가 많아지는 유연한 방식입니다. **Gemini와 같은 최신 모델들이 주로 사용하는 핵심 전략입니다.**
* **Temperature**: 룰렛의 '예측 불가능성'을 조절합니다. 온도가 높으면(\>1.0) 확률이 낮은 단어도 과감하게 뽑아 창의적인 문장이 나오고, 낮으면(\<1.0) 확률 높은 단어 위주로 뽑아 안정적인 문장이 나옵니다.

이 과정을 통해 한 단어를 선택하고, 그 단어를 다시 입력에 추가하여 다음 단어를 예측하는 과정을 반복하면 비로소 하나의 문장이 완성됩니다.
