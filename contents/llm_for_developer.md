...
예측된 토큰을 다시 입력값에 추가하고, 4~6단계를 반복하여 다음 단어를 계속 생성합니다. 이 과정을 `EOS(end of sequence)` 토큰이 생성될 때까지 반복하여 최종적으로 완성된 문장을 사용자에게 반환합니다.
...
- 'LLM의 내부 동작 원리는' 다음에 '매우', '복잡', '정말' 등의 토큰을 적용
