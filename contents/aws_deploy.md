...
Gradle, Maven에서 `Jib plugin`을 활용해 간편하게 이미지를 빌드하고 푸시하는 방법을 알아 보려고 합니다.
...
- 다른 포트도 서비스에 사용한다면 인바운드 탭에서 오픈시켜 주면 됩니다.
...
- ⚠️ ssh(22) 포트나 애플리케이션 관리에 사용되는 포트는 로컬 PC IP로 설정해 두는 게 보안상 필수입니다.
...
이제 도커 허브에 업로드한 이미지로 EC2 인스턴스에 서비스를 실행시켜 볼 차례입니다.
...
이제 `http://[탄력적 IP]:8080`로 접속해 보면 성공적으로 서비스가 실행 중인 것을 확인할 수 있습니다.
...
IS_BLUE=$(docker ps | grep blue) # 실행 중인 컨테이너가 blue인지 확인
...
`cloudflare`에서 도메인을 구매했다면 무료로 SSL 인증서를 무료로 제공하고 만료 전 자동으로 갱신해주고 있습니다.

다른 곳에서 도메인을 구매하고, SSL 인증서가 필요하다면 아래 게시글을 참고해 주세요

[AWS EC2 Amazon Linux 2023 SSL (Certbot, Let’s encrypt)](https://data-make.tistory.com/783)
