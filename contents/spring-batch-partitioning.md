...
|---|---|---|
|조회 방식	| 페이지 단위로 데이터를 끊어서 조회<br/>skip()과 limit() 사용|DB 서버와 커서를 유지하며 스트리밍 방식으로 데이터를 한 건씩 호출|
...
### MongoCursorItemReader 동작 원리

전통적인 `MongoPagingItemReader`가 데이터를 "**페이지 단위**(N개씩 끊어서 여러 번 요청)"로 가져온다면, `MongoCursorItemReader`는 "**데이터 스트림**(열린 연결을 통해 계속 흘려보냄)" 방식을 취합니다.
...
2️⃣. **Cursor 오픈**  
MongoDB 서버에 쿼리를 날리고 빨대(Stream)를 꽂는 단계
...
- `cursor.next()` 호출 시, 클라이언트 메모리 버퍼가 비어있다면 내부적으로 드라이버가 서버에 getMore 명령을 날려 다음 데이터를 스트리밍
...
- Ordered (default): 목록 순서대로 실행. 중간에 에러 나면 중단.
- Unordered: 순서 상관없이 병렬 실행 가능. 에러가 나도 나머지는 진행.
