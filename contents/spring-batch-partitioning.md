...
처음 대용량의 데이터를 처리해야 했을 때 OOM을 마주할 것이라는 기대는 했지만, 만남이 길어질 줄은 몰랐어요. 하지만 문제를 하나씩 분해하고 Spring Batch가 제공하는 도구들을 적재적소에 활용하면서, 결국 '메모리'라는 한정된 자원 안에서 거대한 데이터를 안전하게 통과시키는 법을 배울 수 있었어요

이번 경험의 핵심을 다시 한번 정리하자면 다음과 같아요
- 쪼개기(Partitioning): 거대한 산을 넘기 위해 산을 조각내어 관리 가능한 단위로 만들어요.
- 흘려보내기(CursorReader): 데이터를 쌓아두지 않고 파이프라인처럼 흐르게 하여 메모리 부하를 원천 차단해요.
- 몰아치기(Bulk Operations): 효율적인 일괄 처리를 통해 데이터가 시스템에 머무는 시간을 최소화해요.

단순히 병렬 스레드 수만 늘린다고 해서 성능이 선형적으로 증가하지는 않아요. 오히려 제어되지 않은 병렬성은 OOM이라는 부메랑이 되어 돌아오기도 해요. 중요한 것은 데이터의 유입부터 소멸까지의 전체 생명 주기를 시스템 리소스에 맞춰 세밀하게 설계하는 것임을 다시 한번 깨닫는 계기가 되었어요.

저와 비슷하게 대용량 데이터 처리라는 과제를 안고 고민하는 분들에게 이 글이 조금이나마 도움이 되기를 바랍니다. 기술적인 정답은 상황마다 다르겠지만, **'리소스를 어떻게 효율적으로 사용할 것인가'**에 대한 고민은 언제나 좋은 해답의 시작점이 될 것입니다.