...
`IntentGate`는 사용자의 진짜 의도를 먼저 분석한 뒤 분류하고 행동한다. 
...
- 컨텍스트는 낭비되고, 시간도 낭비된다. `IntentGate`는 이 첫 단계를 확실히 잡는다.
...
OmO의 Hash-Anchored Edit Tool(Hashline)은 이 문제를 콘텐츠 해시로 해결한다. (oh-my-pi에서 영감을 받아 구현)

동작 방식: 에이전트가 파일을 읽으면 각 줄에 해시 태그가 붙는다.
